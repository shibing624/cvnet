...
from cvnet.models.seg.sim_data import generate_random_data

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')


def train(num_epochs=50, show_vgg_params=False, data_dir='', model_save_dir='./checkpoints_simulate/'):
    input_images, target_masks = generate_random_data(192, 192, 3)

    train_dataloader = (torch.from_numpy(input_images), torch.from_numpy(target_masks))
    test_dataloader = (torch.from_numpy(input_images), torch.from_numpy(target_masks))[:100]
